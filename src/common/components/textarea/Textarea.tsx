...
export const Textarea = <TFormValues extends Record<string, unknown>>
  ({ className,  label, disabled, error, register, name, rules, ...rest } : TextareaTypes<TFormValues>) => {
  ...
  return (
    <>
      ...
      <textarea className={classNames(stylesForTextarea, className)} {...(register && register(name, rules))} {...rest} />
      {disabled ?
        null
